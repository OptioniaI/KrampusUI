...
async function GetCookies() {
    ...
    const cwindow = new electron.BrowserWindow({
        ...
        minHeight: 450,
        minWidth: 500,
        title: "Login to your acedia.gg account"
    })

    cwindow.webContents.on("did-navigate-in-page", async (_, url) => {
        if (url !== "https://acedia.gg/dashboard") return;
    ...
    await cwindow.loadURL("https://acedia.gg/", {
...
server.on('connection', socket => {
    APP_SOCKET = socket;
    loaderClient.connect(`wss://acedia.gg/?login_token=%22${API_KEY}%22`, "echo-protocol", "https://acedia.gg/", { cookie: LOADER_COOKIES });
...
loaderClient.on("connect", connection => {
    console.log("Connected to acedia.gg");
